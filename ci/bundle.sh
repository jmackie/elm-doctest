...
mkdir -vp bundle/build/elm-doctest
...
cp -v "$FULL_BIN" bundle/build/elm-doctest

feedback "Copying extra files to staging directory"
cp -v LICENSE bundle/build/elm-doctest
echo "stack ls dependencies >bundle/build/elm-doctest/dependencies.txt"
stack ls dependencies >bundle/build/elm-doctest/dependencies.txt

feedback "Creating $OS.tar.gz"
pushd bundle/build >/dev/null
tar -zcvf "../$OS.tar.gz" elm-doctest
popd >/dev/null
